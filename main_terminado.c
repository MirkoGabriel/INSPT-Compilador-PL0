...
typedef enum {NULO, FIN_DE_ARCHIVO, BEGIN, END, ABREPAREN, CIERRAPAREN, READLN, WRITE, WRITELN, VAR, CADENA, IDENT, NUMERO, PUNTO, COMA, PTOCOMA, MAS,
               CONST, SHL,IGUAL,NOT,MAYOR,EXIT,INC,MENOR, FOR,PROCEDURE, ASIGNAR, CALL, IF,ELSE, THEN, WHILE, DO, ODD, DISTINTO, MENORIGUAL, MAYORIGUAL,MENOS, MULTIPLICAR, DIVIDIR } tTerminal;
...
tSimbolo proposicion (tSimbolo s, FILE *archivo, tablaDeIdent tabla,int base, int posUltimoIdent,byte memoria[],int *tope,int * contadorVariables){
    int p, posPrevCond, posPostCond, origSalto,posPrevProp, destSalto, dist,origSaltoElse,destSaltoElse;
    ...
    case IDENT:
        ...
        s=aLex(archivo);
        if(s.simbolo==ASIGNAR){
                s=aLex(archivo);
        }else if(s.simbolo==IGUAL){
            s=aLex(archivo);
        }
    ...
    case IF:
       ...
       origSalto=*tope;
        ...
        s = proposicion(s,archivo,tabla,base,posUltimoIdent,memoria, tope,contadorVariables);
        destSalto=*tope;
        cargarIntEn(destSalto-origSalto,memoria,origSalto-4);//cargamos la distancia en la posiciòn que sigue a e9
    ...
    case SHL:
        ...
        cargarByte(0x50,memoria,tope);                  //PUSH EAX -> Inserto EAX en la pila

        //Multiplico por 2 (shift left)
        cargarByte(0xB8,memoria,tope);                  //MOV, EAX, 02 00 00 00 (cargo un 2 en EAX)
        cargarInt(2,memoria,tope);



        //cargarPopEax(memoria,tope); // pop Eax
        cargarByte(0x5B,memoria,tope); //pop EBX
        cargarByte(0xF7,memoria,tope); //IMUL EBX (Multiplica EAX y EBX y lo pone en EAX)
        cargarByte(0xEB,memoria,tope); //...
        //cargarByte(0x50,memoria,tope);//PUSH EAX

        //Actualizo la variable en memoria copiando EAX a la dirección EDI + tabla[p].valor
        cargarByte(0x89,memoria,tope);      //MOV [EDI + ....],EAX
        cargarByte(0x87,memoria,tope);      //......
        cargarInt(tabla[p].valor,memoria,tope);   //Offset de memoria respecto de EDI
    ...
    case INC :
        ...
        cargarByte(0x8B,memoria,tope);
        cargarByte(0x87,memoria,tope);
        cargarInt(tabla[p].valor,memoria,tope);
        cargarByte(0x50,memoria,tope);

        cargarByte(0xB8,memoria,tope);
        cargarInt(1,memoria,tope);

        cargarByte(0x5B,memoria,tope);
        /*cargarByte(0x93,memoria,tope);
        cargarByte(0x99,memoria,tope);*/
        cargarByte(0x01,memoria,tope);
